...
## Create alternative response variables
Revenue as percent of budget, indicator variable for if break even or not

```{r alt_response}
movie <- movie %>%
  mutate(rev_budget = real_gross / real_budget,
         profit_dum = ifelse(rev_budget > 1, 1, 0),
         profit_dum = ifelse(rev_budget <= 1, 0, profit_dum))
```
