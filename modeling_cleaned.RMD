...
```{r}
pdf('~/DS5110/project/output/model_plots.pdf')
...
label(train$real_budget) <- 'Real Budget ($)'
label(train$real_gross) <- 'Real Gross Revenue ($)'
label(train$real_budget_log) <- 'Log Real Budget ($)'
label(train$real_gross_log) <- 'Log Real Gross Revenue ($)'
label(train$imdb_score) <- 'IMDB Score'
label(train$imdb_score_log) <- 'Log IMDB Score'
label(train$content_rating) <- 'Content Rating'
label(train$year) <- 'Year'

label(test$real_budget) <- 'Real Budget ($)'
label(test$real_gross) <- 'Real Gross Revenue ($)'
label(test$real_budget_log) <- 'Log Real Budget ($)'
label(test$real_gross_log) <- 'Log Real Gross Revenue ($)'
label(test$imdb_score) <- 'IMDB Score'
label(test$imdb_score_log) <- 'Log IMDB Score'
label(test$content_rating) <- 'Content Rating'
label(test$year) <- 'Year'
```
...
```{r genre_output}
stargazer(mod_genre, type = 'latex', out = 'C:/Users/Gimli/Documents/DS5110/project/output/genre_mod.tex',
          no.space = T, nobs = T, dep.var.caption = '', align = T, dep.var.labels = '', title = 'Genre Model Summary \\\\ Response Variable: Log Real Gross Revenue',
          covariate.labels = c('Adventure', 'Action', 'Family', 'Mystery', 'Romance', 'Drama', 'History', 'Documentary'),
          notes.align = 'l', column.sep.width = "-1pt") 
```
...
train_resid %>% ggplot(aes(sample = lresid)) + 
  geom_qq() +
  geom_qq_line() + 
  labs(title = 'Residual QQPlot: Significant deviations at tails', 
...
lapply(genre_xvar, function(var) {
  train_pred %>% 
    ...
    geom_point(data = train_pred %>% group_by(!!rlang::sym(var)) %>% summarise(mean = mean(pred)),
               aes(y = mean), color = 'red', size = 2) +
    scale_y_log10() + 
    labs(y = 'Log Real Gross Revenue ($)', title = 'Real Revenue Actual vs Predicted\n Successful Prediction') 
})
...
```
Display a couple of plots for presentation. 
...
lapply(c('Adventure', 'Documentary'), function(var) {
  train_pred %>% 
    ...
    geom_boxplot(aes(y = real_gross)) + 
    # include mean
    stat_summary(aes(y = real_gross, color = 'Actual Mean'), fun.y = mean, geom = 'point', size = 4) +
    geom_point(data = train_pred %>% group_by(!!rlang::sym(var)) %>% summarise(mean = mean(pred)),
               aes(y = mean, color = 'Predicted Mean'), size = 2) +
    scale_y_log10() + 
    labs(y = 'Log Real Gross Revenue ($)', title = 'Real Revenue Actual vs Predicted:\nSuccessful Prediction') +
    scale_color_manual(name = '', values = c('Predicted Mean' = 'red', 'Actual Mean' = 'black'))
})
...
train %>% 
  ...
  scale_x_log10() + 
  labs(x = 'Log Real Gross Revenue ($)', y = 'Count', title = 'Histogram Log Real Revenue Actual vs Predicted: Not Complete') + 
  scale_color_manual(name = '', values = c(Actual = 'black', Predicted = 'red'))
```
...
lapply(c('real_budget', 'director_facebook_likes', 'cast_total_facebook_likes', 
  ...
  train_resid %>% 
    ggplot() +  
    geom_point(aes_string(str_c(var, '_log'), y = 'lresid'))
    #labs('Low Budget Movies Over-Predicted; High Budget Under-Predicted')
})
...
lapply(c('real_budget', 'imdb_score'), function(var) {
  train_resid %>% 
    ggplot() + 
    geom_point(aes_string(var, y = 'lresid')) + 
    geom_hline(aes(yintercept = 0)) +
    labs(x = label(train_resid[str_c(var, '_log', sep = '')]), y = 'Log Residual', title = 'Low Budget Movies Over-Predicted; High Budget Under-Predicted') +
    scale_x_log10() 
})
```
...
mod_all <- lm(real_gross_log ~ Adventure + Action + Family + Mystery + 
                  Documentary + Drama + History + Romance  + 
                  real_budget_log + imdb_score_log + content_rating + year, 
          data = train)
...
```{r mod_all_output}
stargazer(mod_all, type = 'latex', out = 'C:/Users/Gimli/Documents/DS5110/project/output/all_mod.tex',
          no.space = T, nobs = T, dep.var.caption = '', align = T, dep.var.labels = '', 
          title = 'Extended Genre Model Summary \\\\ Response Variable: Log Real Gross Revenue',
          omit = c('1981', '1982', '1983', '1984', '1985', '1986', '1987', '1988', '1989', '1990', '1991', '1992', '1993', 
          '1994', '1995', '1996', '1997', '1998', '1999', '2000', '2001', '2002', '2003', '2004', '2005', 
          '2006', '2007', '2008', '2009', '2010', '2011', '2012', '2013', '2014', '2015', '2016'),
          covariate.labels = c('Adventure', 'Action', 'Family', 'Mystery', 'Documentary', 'Drama', 'History', 'Romance', 
                               'Log Real Budget', 'Log IMDB Score', 'NC17', 'PG', 'PG13', 'R'),
          notes = 'Year dummies omitted', notes.align = 'l', column.sep.width = "-1pt") 
```


```{r mod_all_output_anova}
train_labels <- train %>%
  rename(`Log Real Budget ($)` = real_budget_log, 
         `Content Rating` = content_rating,
         `Log IMDB Score` = imdb_score_log,
         Year = year)
mod_all_labels <- lm(real_gross_log ~ Adventure + Action + Family + Mystery + 
                  Documentary + Drama + History + Romance  + 
                  `Log Real Budget ($)` + `Log IMDB Score` + `Content Rating` + Year, 
          data = train_labels)
stargazer(anova(mod_all_labels), type = 'latex', out = 'C:/Users/Gimli/Documents/DS5110/project/output/all_mod_anova.tex',
          no.space = T, nobs = T, dep.var.caption = '', align = T, dep.var.labels = '', 
          title = 'Extended Genre Model Anova \\\\ Response Variable: Log Real Gross Revenue', summary = F)
```
...
```{r mod_all_resid_show, echo = F, results = 'hide', warning = F, message = F}
train_resid_all <- train %>% 
  add_residuals(mod_all, 'lresid')

# labels aren't persisting for some reason  - make sure 
label(train_resid_all$real_budget_log) <- 'Log Real Budget ($)'
label(train_resid_all$imdb_score_log) <- 'Log IMDB Score'
...
lapply(c('real_budget', 'imdb_score'), function(var) {
  train_resid_all %>% 
    ...
    geom_point(aes_string(var, y = 'lresid')) + 
    geom_hline(aes(yintercept = 0)) +
    labs(x = label(train_resid_all[str_c(var, '_log', sep = '')]), y = 'Log Residual') +
    scale_x_log10() + labs(title = 'Random Relationship')
})
```
...
train %>% 
  ...
  scale_x_log10() + 
  theme(legend.position="bottom") + 
  labs(x = 'Log Real Gross Revenue ($)', y = 'Count', title = 'Histogram Log Real Revenue Actual vs Predicted') + 
  scale_color_manual(name = '', values = c(Actual = 'black', Predicted = 'red'))
```
...
train %>% 
  ...
  geom_freqpoly(aes(x = pred, color = 'Predicted')) +
  scale_x_log10() + 
  labs(x = 'Log Real Gross Revenue ($)', y = 'Count', title = 'Histogram Log Real Revenue Actual vs Predicted: Budget Only') + 
  scale_color_manual(name = '', values = c(Actual = 'black', Predicted = 'red'))
```
...
lapply(c('content_rating'), function(var) {
  train_resid %>%
    filter(!is.na(!!rlang::sym(var))) %>%
    filter(lresid < 2.5) %>%
    ggplot(aes_string(var, 'lresid')) + 
    geom_jitter(alpha = .3) + 
    geom_point(data = train_resid %>% filter(lresid > 2.5), color = 'red') +
    geom_text_repel(aes(label = movie_title), data = label, nudge_x = -.6, segment.color = NA) +
    labs(x = label(train_resid[var]), y = 'Log Residual', title = 'Log Residual Plot: Outliers')
}) 

lapply(c('real_budget', 'imdb_score'), function(var) {
  train_resid %>% 
    filter(lresid < 2.5) %>%
    ggplot(aes_string(str_c(var, '_log'), y = 'lresid')) + 
    geom_point() +
    geom_point(data = train_resid %>% filter(lresid > 2.5), color = 'red') +
    geom_text_repel(aes(label = movie_title), data = label, segment.color = NA) +
    labs(x = label(train_resid[var]), y = 'Log Residual', title = 'Log Residual Plot: Outliers')
})
...
*  Somewhat of a toss-up between model without genre and model with some genre variables. Predictions similar, random relationships with resiudals.
  ...
  *  Genres are captured by other variables: for example, genre and budget vary together significantly, as well as genre and content rating, so those effects are captured. If I was a movie executive, picking genre is still tied up in this because that likely effects the budget and content rating, which are significant and strong predictors. 
      *  See graphs at end of EDA: compare genre and budget and content rating. For budget, ranking of genre almost identical to ranking based off of revenue
...
```{r mod_test}
mod_simple2 <- lm(real_gross_log ~ real_budget_log + imdb_score_log + content_rating + year,
               data = test)
...
mod_simple3 <- lm(real_gross_log ~ real_budget_log,
               data = test)

summary(mod_simple3)
anova(mod_simple3)
rmse(mod_simple3, data = test) 

```

```{r output}
stargazer(mod_simple2, type = 'latex', out = 'C:/Users/Gimli/Documents/DS5110/project/output/final_mod.tex',
          no.space = T, nobs = T, dep.var.caption = '', align = T, dep.var.labels = '', 
          title = 'Final Model Summary \\\\ Response Variable: Log Real Gross Revenue',
          omit = c('1981', '1982', '1983', '1984', '1985', '1986', '1987', '1988', '1989', '1990', '1991', '1992', '1993', 
          '1994', '1995', '1996', '1997', '1998', '1999', '2000', '2001', '2002', '2003', '2004', '2005', 
          '2006', '2007', '2008', '2009', '2010', '2011', '2012', '2013', '2014', '2015', '2016'),
          covariate.labels = c('Log Real Budget', 'Log IMDB Score', 'NC17', 'PG', 'PG13', 'R'),
          notes = 'Year dummies omitted for presentation purposes', notes.align = 'l', column.sep.width = "-1pt") 

# stargazer anova doesn't let you do nice variable labels: covariate.labels is for column headers in this case
test_labels <- test %>%
  rename(`Log Real Budget ($)` = real_budget_log, 
         `Content Rating` = content_rating,
         `Log IMDB Score` = imdb_score_log,
         Year = year)
mod_simple2_labels <- lm(real_gross_log ~ `Log Real Budget ($)` + `Log IMDB Score` + `Content Rating` + Year,
               data = test_labels)
stargazer(anova(mod_simple2_labels), type = 'latex', out = 'C:/Users/Gimli/Documents/DS5110/project/output/final_mod_anova.tex',
          no.space = T, nobs = T, dep.var.caption = '', align = T, dep.var.labels = '', 
          title = 'Final Model Model Anova \\\\ Response Variable: Log Real Gross Revenue', summary = F)
         
          # covariate.labels = c('Log Real Budget', 'Log IMDB Score', 'NC17', 'PG', 'PG13', 'R', '1981', 
          #'1982', '1983', '1984', '1985', '1986', '1987', '1988', '1989', '1990', '1991', '1992', '1993', 
          #'1994', '1995', '1996', '1997', '1998', '1999', '2000', '2001', '2002', '2003', '2004', '2005', 
          #'2006', '2007', '2008', '2009', '2010', '2011', '2012', '2013', '2014', '2015', '2016', 'Constant'))
```
...
test %>% 
  ...
  scale_x_log10() + 
  theme(legend.position="bottom") + 
  labs(x = 'Log Real Gross Revenue ($)', y = 'Count', title = 'Histogram Log Real Revenue Real vs Predicted') + 
  scale_color_manual(name = '', values = c(Actual = 'black', Predicted = 'red'))
```
