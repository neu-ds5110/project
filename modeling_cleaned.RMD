...
lapply(c('Adventure', 'Documentary'), function(var) {
  train_pred %>% 
    ...
    geom_boxplot(aes(y = real_gross)) + 
    # include mean
    stat_summary(aes(y = real_gross, color = 'Mean'), fun.y = mean, geom = 'point', size = 4) +
    geom_point(data = train_pred %>% group_by(!!rlang::sym(var)) %>% summarise(mean = mean(pred)),
               aes(y = mean, color = 'Predicted'), size = 2) +
    scale_y_log10() + 
    labs(y = 'Log Real Gross Revenue', title = 'Revenue Actual vs Predicted: Log Scale \n Successful Prediction') +
    scale_color_manual(name = '', values = c('Predicted' = 'red', 'Mean' = 'black'))
})
...
# graph each against log residual: continuous (log scale)
lapply(c('real_budget_log', 'imdb_score_log'), function(var) {
  train_resid %>% 
    ggplot() + 
    geom_point(aes_string(var, y = 'lresid')) + 
    geom_hline(aes(yintercept = 0)) +
    labs(x = label(train_resid[var]), y = 'Log Residual')
})
...
stargazer(mod_simple2, type = 'latex', out = 'C:/Users/Gimli/Documents/DS5110/project/output/final_mod.tex',
          no.space = T, nobs = T, dep.var.caption = '', align = T, dep.var.labels = '', title = 'Final Model Summary',
