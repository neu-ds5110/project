...
```{r log}
train <- train %>% 
  mutate_at(vars(real_gross, real_budget, director_facebook_likes, cast_total_facebook_likes, imdb_score), funs(log = log(.)))
```
## Oscars 
...
oscar_box <- function(df, var, title_str, x_str) {
  print(ggplot(df, aes_string(var, "real_gross")) + 
    geom_boxplot() + 
    labs(title = title_str, x = x_str, y = 'Real Gross Revenue ($)'))
  print(ggplot(df, aes_string(var, "real_gross_log")) + 
    geom_boxplot() + 
...
oscar_bar <- function(df, var, title_str, x_str) {
  ...
  print(ggplot(df, aes_string(var, "avg_real_gross_log")) + 
    geom_col() + 
    labs(title = title_str, x = x_str, y = 'Average Log Real Gross Revenue ($)'))
}
...
oscar_bar(train_oscar_actor, 'total_oscars_actor', 'Total # Oscars of Actors vs Real Gross Revenue', 
          'Total # Oscars Held by Actors in Movie vs Log Real Revenue')
...
Not as strong a relationship as I originally expected. Not sure this is worth including either.
...
facebook_plot <- function(xvar, xlab) {
 ...
 print(train %>%
    ggplot(aes_string(x = str_c(xvar, '_log'), y = 'real_gross')) + 
    geom_point() + 
    geom_smooth() + 
    labs(x = str_c('Log ', xlab), y = 'Real Gross Revenue ($)'))
 print(train %>%
    ggplot(aes_string(x = xvar, y = 'real_gross_log')) + 
    geom_point() + 
    geom_smooth() + 
    labs(x = xlab, y = 'Log Real Gross Revenue ($)'))
 print(train %>%
    ggplot(aes_string(x = str_c(xvar, '_log'), y = 'real_gross_log')) + 
    geom_point() + 
    geom_smooth() + 
...
train_sum <- train %>%
  ...
  summarize(real_gross_avg = mean(real_gross),
            real_gross_avg_log = mean(real_gross_log))
...
plt <- ggplot(data = train_sum %>% filter(year >= 1995))
plt + 
  ...
  geom_rect(aes(xmin = 2001, xmax = 2002, 
                ...
                ymax = max(real_gross_avg, na.rm = T)), alpha = .05) 
plt + 
  geom_line(aes(x = year, y = real_gross_avg_log)) +
  labs(title = 'Average Log Real Gross Revenue Over Time', x = 'Year', y = 'Average Log Real Gross Revenue ($)') 
...
train_content  %>% 
  ggplot() + 
  geom_boxplot(aes(x = content_rating, y = real_gross_log, fill = content_rating)) +
...
train_content  %>% 
  ...
  labs(x = 'Content Rating', y = 'Average Log Real Gross Revenue ($)', 
...
lapply(unique(train_content$content_rating), function(r) {
  train_content %>% 
    ...
    labs(x = r, y = 'Average Log Real Gross Revenue ($)')
})
...
train_genre %>% 
  ...
  coord_flip() + 
  labs(x = 'Genre', y = 'Average Log Real Gross Revenue ($)',
...
train_genre %>%
  ggplot() + 
  geom_boxplot(aes(x = genre, y = real_gross_log, fill = genre)) + 
  coord_flip() + 
